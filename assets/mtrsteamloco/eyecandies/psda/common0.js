...
function texture(entity, state, a) {
    ...
    for (let i = 0; i < mins.length; i++) {
        ...
        for (let i = 0; i < 2; i++) {
            let tex = new GraphicsTexture(700/2, 1100/2);
            let g = tex.graphics;
            g.setColor(new Color(state.colorA));
            g.fillRect(0, 0, 700/2, 1100/2);
            //try {
                font1 = fontb.deriveFont(Font.PLAIN, 140/2);
                g.setFont(font1);
                g.setColor(new Color(state.colorC));
                g.drawString(rn, 700/2/2 - getSize(rn, font1)[0]/2, 160/2);
            
                font2 = fontb.deriveFont(Font.PLAIN, 110/2);
                g.setFont(font2);
                g.drawString(sn, 700/2/2 - getSize(sn, font2)[0]/2, 420/2);
                g.drawString(en, 700/2/2 - getSize(en, font2)[0]/2, 630/2);
            
                font3 = fontb.deriveFont(Font.BOLD, 80/2);
                g.setFont(font3);
                g.setColor(new Color(state.colorB));
                let str = "< 往 >";
                g.drawString(str, 700/2/2 - getSize(str, font3)[0]/2, 510/2);
            
                font4 = fontb.deriveFont(Font.PLAIN, 60/2);
                g.setFont(font4);
                g.setColor(new Color(state.colorC));
                str = entity.data.get("slogan" + (i == 0 ? "A1" : "B1")) + "";
                g.drawString(str, 700/2/2 - getSize(str, font4)[0]/2, 900/2);
            
                str = entity.data.get(("slogan" + (i == 0 ? "A2" : "B2"))) + "";
                g.drawString(str, 700/2/2 - getSize(str, font4)[0]/2, 1000/2);
    
                font5 = fontb.deriveFont(Font.PLAIN, 65/2);
                g.setFont(font5);
                g.setColor(new Color(state.colorC));
                str = state.dt + "到, " + state.ft + "开";
                g.drawString(str, 700/2/2 - getSize(str, font5)[0]/2, 750/2);
    
                font6 = fonta.deriveFont(Font.PLAIN, 65/2);
                str = entity.data.get("slogan0") + "";
                g.setFont(font6);
                g.drawString(str, 700/2/2 - getSize(str, font6)[0]/2, 260/2);
                state.sc = true;
