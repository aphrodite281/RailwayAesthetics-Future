...
const font0 = Resources.readFont(Resources.id("mtrsteamloco:library/font/hkh_sxt.ttf")).deriveFont(Font.PLAIN, 76)//Resources.getSystemFont("Noto Sans").deriveFont(Font.PLAIN, 70);
...
function create(ctx, state, entity) {
    ...
    size[1] = 80;
    let h = size[1]/400;
    ctx.setDebugInfo(size[0],size[1])
    ctx.setDebugInfo(h,h)
    let info = {
        ctx: ctx,
        uvSpeed: [100, 0],
        model: {
    ...
    g.drawString(str, size[0]/2-getSize(str, font0)[0]/2, 80);
    scs.texture.upload();
